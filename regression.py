...
def vis_linreg_model(train_X, train_Y, Theta):
	sample_X, sample_Y, sample_Z = None, None, None
...
def linreg_closed_form(train_X, train_Y):
	...
	Theta = numpy.linalg.inv(train_X.T @ train_X) @ train_X.T @ train_Y # TODO: compute the closed form solution here. Note: using numpy.linalg.lstsq(...) is *not* the correct answer
	return Theta

def correct_answer(train_X, train_Y):
	return numpy.linalg.lstsq(numpy.hstack((numpy.ones((train_X.shape[0], 1)), train_X)), train_Y, rcond=None)[0]
###################
# YOUR CODE BELOW #
###################       
def loss(Theta, train_X, train_Y):
	...
	'''
	delta_Y = train_X.dot(Theta) - train_Y.reshape((train_X.shape[0], 1))
	rv = (delta_Y.T @ delta_Y) / (2 * len(train_Y)) # TODO: compute the loss here.
	return rv[0][0]
...
def linreg_grad_desc(initial_Theta, train_X, train_Y, alpha=0.1, num_iters=500, print_iters=True):
	...
	'''
	train_X = numpy.hstack((numpy.ones((train_X.shape[0], 1)), train_X))
	cur_Theta = initial_Theta.astype(numpy.float64).reshape((train_X.shape[1], 1))
	step_history = list()
	for k in range(1,num_iters+1):
		...
		cur_Theta -= alpha * (train_X.T @ train_X @ cur_Theta - (train_X.T @ train_Y).reshape((train_X.shape[1],1))) / train_X.shape[0]
	return step_history
...
def random_fourier_features(train_X, train_Y, num_fourier_features=100, alpha=0.1, num_iters=500, print_iters=False):
	...
	for i in range(num_fourier_features):
		randv = numpy.random.random(size=train_X.shape[1]) - 0.5
		Omega.append(randv / numpy.linalg.norm(randv) * numpy.abs(numpy.random.normal()))	# TODO: sample inner-products
		B.append(numpy.random.random() * 2 * numpy.pi)		# TODO: sample translations
	Omega = numpy.array(Omega).T
	B = numpy.array(B)
	...
	initial_Theta = (numpy.random.random(size=num_fourier_features+1)-0.5)*0.2
...
def rff_model_sample(Theta,Omega,B,model_X):
	sampled_X = numpy.linspace(model_X.min(axis=0),model_X.max(axis=0) * 5,100)
	Phi = apply_RFF_transform(sampled_X,Omega,B)
	sampled_Y = numpy.hstack((numpy.ones((Phi.shape[0], 1)), Phi)).dot(Theta)
	return sampled_X, sampled_Y
...
if __name__ == '__main__':
	...
	Theta, Omega, B = random_fourier_features(data_X, data_Y, num_fourier_features=200)
